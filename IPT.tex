...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        \[P\left(\ \bigcup\limits_{k=n}^{\infty}\overline{A_k}\right)\leq\text{exp}{\left(-\sum\limits_{k=n}^{N}P(A_k)\right)}\to 0\]
    \end{enumerate}
\end{proof}
\begin{lemma} (Лемма о групировке)\\
    Пусть имеется семейтво $\mathcal{A}_t,\ t\in I$ независимых $\sigma$-алгебр. Возьмем $I_1,I_2,\dots\subset T: I_k\cap I_m=\emptyset,\ \forall k\ne m$ и для $I\subset T$ введем $\sigma\{I\}=\sigma\{\mathcal{A}_t,\ t\in I\}$. Тогда $\sigma\{I_1\}, \sigma\{I_2\}, \dots$ - независимые $\sigma$-алгербы.
\end{lemma}
\end{document}