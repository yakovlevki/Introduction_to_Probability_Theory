...
\end{definition}
\begin{definition}
    Пусть все $\mathcal{G}_k$ содержат $\Omega$ (например $\mathcal{G}_k$ - алгебры), то это определение равносильно следующему: 
...
\begin{proof}\tab
    \begin{enumerate}
        \item Первое определние влечет второе, поскольку в качестве $i_1,\dots,i_k$ можно взять $1,\dots,n$.
...
\begin{proof}
    ...
    Аналогичными рассуждениями, получим, что $\sigma\{M_2\}\subset D_2, \dots, \sigma\{M_n\}\subset D_n$. Поскольку $D_1,\dots, D_n$ независимы, то $\sigma\{M_1\}, \dots, \sigma\{M_n\}$ независимымы.
\end{proof}
...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        \[P\left(\ \bigcup\limits_{k=n}^{\infty}\overline{A_k}\right)\]
        Поскольку вероятностная мера непрерывна, то 
        \[P\left(\ \bigcup\limits_{k=n}^{\infty}\overline{A_k}\right)=\lim\limits_{N\to\infty}P\left(\ \bigcup\limits_{k=n}^{N}\overline{A_k}\right)\]
        $A_1,\dots,A_n$ независимы $\Rightarrow \overline{A_1}, \dots, \overline{A_n}$ независимы по лемме. Поэтому
        \begin{multline*}
            P\left(\ \bigcup\limits_{k=n}^{N}\overline{A_k}\right)=\prod\limits_{k=n}^{N}P(\overline{A_k})=\prod\limits_{k=n}^{N}(1-P(A_k))\overset{(1)}{\leq}\\
        ...
        \[P\left(\ \bigcup\limits_{k=n}^{\infty}\overline{A_k}\right)\leq\text{exp}{\left(-\sum\limits_{k=n}^{\infty}P(A_k)\right)}\to 0\]
    \end{enumerate}
\end{proof}
\begin{lemma} (Лемма о групировке)\\
    Пусть имеется семейтво $\mathcal{A}_t,\ t\in T$ независимых $\sigma$-алгебр. Возьмем $I_1,I_2,\dots\subset T: I_k\cap I_m=\emptyset,\ \forall k\ne m$ и для $I\subset T$ введем $\sigma\{I\}=\sigma\{\mathcal{A}_t,\ t\in I\}$. Тогда $\sigma\{I_1\}, \sigma\{I_2\}, \dots$ - независимые $\sigma$-алгербы.
\end{lemma}
\section{Лекция 4}
...
\end{consequense}
\begin{consequense}
    Пусть $(V, \tau)$ и $(S, \nu)$ - топологические пространства. Тогда $f: V\to S$ непрерывна $(f^{-1}(\nu)\subset \tau)$. Тогда $f\in \mathcal{B}(V)|\mathcal{B}(S)$
\end{consequense}
\subsection{Распределение случайного элемента}
\begin{definition}
    Рассмотрим вероятностыне пространства $(\Omega, \mathcal{F})$ и $(S, \mathcal{B})$. Пусть $X:\Omega \to S,\ X\in \mathcal{F}|\mathcal{B}$. Распределением случайного элемента $X$ называется вероятностная мера $P_X$ на $(S,\mathcal{B})$ такая, что
    \[P_X(B)=P(X^{-1}(B))\]
...
\begin{definition} (Биномиальное распределение)\\
    $X \sim B_{\epsilon}(n,p)$, где $n\in \N,\ 0<P<1$, если
...
\begin{proof}
    ...
    \[\frac{n!}{n^k(n-k)!}\to 1\]
    Действительно
    \[\frac{n!}{n^k (n-k)!}=\frac{(n-k+1)!}{n^k}\to 1\]
...
\begin{theorem}
    Пусть $X_1, \dots, X_n$ - независимые случайные элементы,\\
...
\begin{proof}
    ...
    \begin{multline*}
        ...
        =\sum\limits_{i=0}^{m}P(Y_1=i,\ Y_2=m-i)=\sum\limits_{i=0}^{m}P(Y_1=i)P(Y_2=m-i)\overset{(2)}{=}\tab[2.5cm]\\
        \tab[1.5cm]\overset{(2)}{=}\sum\limits_{i=0}^{m}\left(\frac{\lambda_1^i e^{-\lambda_1}}{i!}\cdot \frac{\lambda_2^{m-i}e^{-\lambda_2}}{(m-i)!}\right)=\frac{e^{-(\lambda_1+\lambda_2)}}{m!}\cdot \sum\limits_{i=0}^{m} \left(\frac{m!}{i! (m-i)!}\lambda_1^i \lambda_2^{m-i}=\right)=\\
...
\end{proof}
\begin{lemma}
    Пусть $X, Y$ случайные величины. Тогда $\forall B\in \mathcal{B}$ выполнено:
    \[|P(X\in B)-P(Y\in B)|\leq P(X\neq Y)\]
\end{lemma}
\begin{proof}
    Рассмотрим разбиение $\Omega$ на две части: $Y(\omega)\in B$ и $Y(\omega)\not\in B$. Тогда по формуле полной вероятности:
    \begin{multline*}
...
\begin{theorem} (Теорема Ломницкого-Улама)\\
    Пусть $(S_t,\mathcal{B}_t,Q_t)$ - семейтво вероятностных пространств. Тогда на некотором вероятностном пространтсве $(\Omega, \mathcal{F}, P)$ можно построить семейство независимых случайных элементов $X_t: \Omega\to S_t,\ X_t\in \mathcal{F}|\mathcal{B}_t$, причем $P_{X_t}=Q_t,\ t\in T$. Это означает, что всегда можно построить семейство независимых случайных элементов, имеющих заданное распределение.
\end{theorem}
...
\begin{proof}
    ...
    \[X_n(\omega)=\begin{cases}
        ...
        n\ \ \ \ \ , \ \ X(\omega)\geq n.
    \end{cases}\]
    Тогда $X_n$ не убывает и $X_n\to X$.
\end{proof}
\begin{lemma}
...
\end{lemma}
\begin{proof}
    $X_n$ - простые и $X_{n}\leq X_{m+1} \Rightarrow EX_n \leq EX_{n+1}$. При этом
    \[EX=\sup\{EY: 0\leq Y\leq X,\ Y - \text{простая}\}\]
    ...
    \end{cases}\]
    Тогда
    \[EY=\sum\limits_{j=1}^{j}b_j p_j\]
    Введем $Y_n=(1-\epsilon)Y \cdot I\{(1-\epsilon)Y\leq X\},\ n\in \N,\ \epsilon\in (0,1)$. Из определения
    $Y_n\leq X_n \Rightarrow EY_n\leq EX_n\leq a$. Пусть 
    \[B_j=\{\omega: Y(\omega)=b_j\},\ C_{jn}=\{(1-\epsilon)Y\leq X_n\}\]
    Тогда $Y(\omega)=(1-\epsilon)b_j$, если $\omega\in B_j\cap C_{jn}$. Получаем, что
    \[EY_n=\sum\limits_{j}(1-\epsilon)b_j P(C_{jn})\]\
    При этом, по свойству непрерывности вероятностной меры
    \[P(C_{jn})\to P(B_j)\]
    следовательно
    \[EY_n=\sum\limits_{j}(1-\epsilon)b_j P(C_{jn})\to (1-\epsilon)\sum\limits_{j}b_j P(B_{j})=(1-\epsilon)EY\]
...
\begin{proof}
    ...
    \[h(x)=I_B(x)=\begin{cases}
        ...
        0,\ x\not\in B
    \end{cases}\]
    \[\int\limits_{S}h(x)P_X(dx)=1\cdot P_X(B)=P(X\in B)\]
...
\end{comm}
\begin{comm}
    Если $X$ - дискретная случайная велисина, то
    \[\var{X}=\sum\limits_{n}(X_n-EX)^2 P(X=x_n)\]
\end{comm}
\begin{theorem}
    Пусть $X_1, X_2, \dots \in \mathcal{L}^2$. Тогда 
...
\begin{proof}
    \begin{multline*}
        ...
        \overset{(1)}{=}\sum\limits_{i,j=1}^{n}E((X_i-EX_i)(X_j-EX_j))\overset{(3)}{=}
    \end{multline*}
    ...
    (3): Поскольку $U,V\in \mathcal{L}^1$, то из $0\leq |U\cdot V|\leq \frac{1}{2}(U^2+V^2)$ получаем $U\cdot V\in \mathcal{L}^1$
\end{proof}
\end{document}